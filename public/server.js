  ...
  fs.readdir(dirPath, (err, files) => {
    ...
    files.forEach((file, i) => {
      ...
      fs.readFile(`${dirPath}/${file}`, "utf8", (err, contents) => {
        const getMetadataIndices = (acc, ele, i) => {
          if (/^---/.test(ele)) {
            acc.push(i);
          }
          return acc;
        ...
        post = {
          ...
          postedOn: publishedDate ? publishedDate : "No date given",
          author: metadata.author ? metadata.author : "No author given",
          blogImage: metadata.blogImage,
          blogText: content ? content : "No content given",
        };
        postList.push(post);
        ilist.push(i);
        if (ilist.length === files.length) {
          ...
          fs.writeFileSync("src/data/blog.json", data);
        }
      });
    });
  });
